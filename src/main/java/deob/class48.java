package deob;

import java.awt.Component;
import java.awt.Graphics;
import java.awt.Image;

@ObfuscatedName("gf")
public abstract class class48 {

	@ObfuscatedName("gf.b")
	public int field1120;

	@ObfuscatedName("gf.c")
	public Image field1121;

	@ObfuscatedName("gf.d")
	public int field1122;

	@ObfuscatedName("gf.e")
	public static class36 field1123;

	@ObfuscatedName("gf.f")
	public int[] field1124;

	@ObfuscatedName("gf.g")
	public static class63 field1125 = new class63(50);

	@ObfuscatedName("gf.h")
	public static int field1126;

	@ObfuscatedName("gf.k")
	public static class32 field1129 = class73.method593("Zu viele Anmelde)2Versuche von Ihrer Adresse", true);

	@ObfuscatedName("gf.m")
	public static int field1131 = 0;

	@ObfuscatedName("gf.n")
	public static class32 field1132 = class73.method593(" )2> <col=ff9040>", true);

	@ObfuscatedName("gf.o")
	public static class30 field1133;

	@ObfuscatedName("gf.p")
	public static class32 field1134 = class73.method593("Bitte geben Sie Ihren Benutzenamen ein)3", true);

	@ObfuscatedName("gf.r")
	public static class32 field1136 = class73.method593("Loading )2 please wait)3", true);

	@ObfuscatedName("gf.q")
	public static class32 field1135 = field1136;

	@ObfuscatedName("gf.s")
	public static class49 field1137;

	@ObfuscatedName("gf.a(Z)V")
	public static void method363() {
		class37.field886.method490();
	}

	@ObfuscatedName("gf.a(B)V")
	public final void method361() {
		class141.method1052(this.field1124, this.field1120, this.field1122);
	}

	@ObfuscatedName("gf.a(Ljava/awt/Graphics;III)V")
	public abstract void method122(Graphics arg0);

	@ObfuscatedName("gf.a(IIILjava/awt/Component;)V")
	public abstract void method123(int arg0, int arg1, Component arg2);

	@ObfuscatedName("gf.a(ILjava/awt/Graphics;IIII)V")
	public abstract void method124(Graphics arg0, int arg1, int arg2, int arg3, int arg4);
}
