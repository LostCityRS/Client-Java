package deob;

@ObfuscatedName("ta")
public final class class133 extends class13 {

	@ObfuscatedName("ta.db")
	public int field3059 = 0;

	@ObfuscatedName("ta.ib")
	public int field3064 = 0;

	@ObfuscatedName("ta.lb")
	public static int field3067 = 0;

	@ObfuscatedName("ta.ob")
	public int field3070;

	@ObfuscatedName("ta.pb")
	public short[] field3071;

	@ObfuscatedName("ta.qb")
	public static class32 field3072 = class73.method593("Bad session id)3", true);

	@ObfuscatedName("ta.rb")
	public int field3073 = 0;

	@ObfuscatedName("ta.sb")
	public int field3074 = 128;

	@ObfuscatedName("ta.tb")
	public static class32 field3075 = field3072;

	@ObfuscatedName("ta.K")
	public static int field3040 = 0;

	@ObfuscatedName("ta.Y")
	public static int field3054 = 0;

	@ObfuscatedName("ta.X")
	public static class32 field3053 = class73.method593("This world is full)3", true);

	@ObfuscatedName("ta.W")
	public static class32 field3052 = class73.method593("<col=00ffff>", true);

	@ObfuscatedName("ta.Fb")
	public static class32 field3087 = class73.method593("", true);

	@ObfuscatedName("ta.T")
	public static class32 field3049 = field3087;

	@ObfuscatedName("ta.N")
	public static class32 field3043 = field3087;

	@ObfuscatedName("ta.Q")
	public static class32 field3046 = field3087;

	@ObfuscatedName("ta.I")
	public static class32 field3038 = field3087;

	@ObfuscatedName("ta.O")
	public static class32 field3044 = field3087;

	@ObfuscatedName("ta.Db")
	public static class32 field3085 = field3087;

	@ObfuscatedName("ta.V")
	public static class32 field3051 = field3087;

	@ObfuscatedName("ta.Rb")
	public static class32 field3099 = field3087;

	@ObfuscatedName("ta.Lb")
	public static class32 field3093 = field3087;

	@ObfuscatedName("ta.Pb")
	public static class32 field3097 = field3087;

	@ObfuscatedName("ta.L")
	public static class32 field3041 = field3087;

	@ObfuscatedName("ta.vb")
	public static class32 field3077 = field3087;

	@ObfuscatedName("ta.wb")
	public static class32 field3078 = field3087;

	@ObfuscatedName("ta.ab")
	public static class32 field3056 = field3087;

	@ObfuscatedName("ta.bb")
	public static class32 field3057 = field3087;

	@ObfuscatedName("ta.cb")
	public static class32 field3058 = field3087;

	@ObfuscatedName("ta.hb")
	public static class32 field3063 = field3087;

	@ObfuscatedName("ta.jb")
	public static class32 field3065 = field3087;

	@ObfuscatedName("ta.kb")
	public static class32 field3066 = field3087;

	@ObfuscatedName("ta.mb")
	public static class32 field3068 = field3087;

	@ObfuscatedName("ta.nb")
	public static class32 field3069 = field3087;

	@ObfuscatedName("ta.zb")
	public static int field3081 = 0;

	@ObfuscatedName("ta.H")
	public static class32 field3037 = field3087;

	@ObfuscatedName("ta.Qb")
	public static class32 field3098 = field3087;

	@ObfuscatedName("ta.Nb")
	public static class32 field3095 = field3087;

	@ObfuscatedName("ta.yb")
	public static class32 field3080 = field3087;

	@ObfuscatedName("ta.Kb")
	public static class32 field3092 = field3087;

	@ObfuscatedName("ta.Bb")
	public static class32 field3083 = field3087;

	@ObfuscatedName("ta.Ib")
	public static class32 field3090 = field3087;

	@ObfuscatedName("ta.Ab")
	public static int field3082 = 128;

	@ObfuscatedName("ta.U")
	public static class32 field3050 = field3087;

	@ObfuscatedName("ta.P")
	public static class32 field3045 = class73.method593(" is already on your friend list", true);

	@ObfuscatedName("ta.ub")
	public static class32 field3076 = field3045;

	@ObfuscatedName("ta.S")
	public static class32 field3048 = field3053;

	@ObfuscatedName("ta.M")
	public static class32 field3042 = field3087;

	@ObfuscatedName("ta.Jb")
	public static class32 field3091 = field3087;

	@ObfuscatedName("ta.Sb")
	public static class32 field3100 = field3087;

	@ObfuscatedName("ta.Cb")
	public static class32 field3084 = field3087;

	@ObfuscatedName("ta.Xb")
	public static class32 field3105 = class73.method593("Try again in 60 secs)3)3)3", true);

	@ObfuscatedName("ta.Yb")
	public static class32 field3106 = field3087;

	@ObfuscatedName("ta.Gb")
	public static class32 field3088 = field3105;

	@ObfuscatedName("ta.Tb")
	public static class32 field3101 = field3087;

	@ObfuscatedName("ta.Mb")
	public static class32 field3094 = field3087;

	@ObfuscatedName("ta.Zb")
	public static class32 field3107 = field3087;

	@ObfuscatedName("ta.Ob")
	public int field3096 = -1;

	@ObfuscatedName("ta.xb")
	public int field3079 = 128;

	@ObfuscatedName("ta.J")
	public int field3039;

	@ObfuscatedName("ta.R")
	public static class152 field3047;

	@ObfuscatedName("ta.Eb")
	public static int[] field3086;

	@ObfuscatedName("ta.Z")
	public short[] field3055;

	@ObfuscatedName("ta.Ub")
	public short[] field3102;

	@ObfuscatedName("ta.Wb")
	public short[] field3104;

	@ObfuscatedName("ta.e(I)V")
	public static void method1012() {
		field3065 = null;
		field3042 = null;
		field3056 = null;
		field3087 = null;
		field3106 = null;
		field3058 = null;
		field3053 = null;
		field3043 = null;
		field3100 = null;
		field3084 = null;
		field3083 = null;
		field3066 = null;
		field3097 = null;
		field3076 = null;
		field3099 = null;
		field3049 = null;
		field3052 = null;
		field3088 = null;
		field3037 = null;
		field3086 = null;
		field3080 = null;
		field3098 = null;
		field3105 = null;
		field3057 = null;
		field3091 = null;
		field3068 = null;
		field3093 = null;
		field3078 = null;
		field3069 = null;
		field3090 = null;
		field3077 = null;
		field3101 = null;
		field3075 = null;
		field3051 = null;
		field3095 = null;
		field3038 = null;
		field3045 = null;
		field3048 = null;
		field3072 = null;
		field3063 = null;
		field3041 = null;
		field3046 = null;
		field3050 = null;
		field3047 = null;
		field3107 = null;
		field3092 = null;
		field3044 = null;
		field3085 = null;
		field3094 = null;
	}

	@ObfuscatedName("ta.a(IIIZII)Lh;")
	public static class49 method1016(int arg0, int arg1, boolean arg2, int arg3, int arg4) {
		long var5 = ((long) arg3 << 40) + ((long) arg0 << 38) + (long) arg1 + ((long) arg4 << 16);
		if (!arg2) {
			class49 var7 = (class49) class47.field1114.method485(var5);
			if (var7 != null) {
				return var7;
			}
		}
		class62 var8 = class146.method1092(arg1);
		if (arg4 > 1 && var8.field1412 != null) {
			int var9 = -1;
			for (int var10 = 0; var10 < 10; var10++) {
				if (arg4 >= var8.field1372[var10] && var8.field1372[var10] != 0) {
					var9 = var8.field1412[var10];
				}
			}
			if (var9 != -1) {
				var8 = class146.method1092(var9);
			}
		}
		class101 var11 = var8.method470(1);
		if (var11 == null) {
			return null;
		}
		class49 var12 = null;
		if (var8.field1388 != -1) {
			var12 = method1016(1, var8.field1411, true, 0, 10);
			if (var12 == null) {
				return null;
			}
		}
		int[] var13 = class141.field3237;
		int var14 = class141.field3240;
		int var15 = class141.field3235;
		int[] var16 = new int[4];
		class141.method1070(var16);
		class49 var17 = new class49(36, 32);
		class141.method1052(var17.field1144, 36, 32);
		class141.method1054();
		class7.method36();
		class7.method43(16, 16);
		int var18 = var8.field1391;
		class7.field118 = false;
		if (arg2) {
			var18 = (int) ((double) var18 * 1.5D);
		} else if (arg0 == 2) {
			var18 = (int) ((double) var18 * 1.04D);
		}
		int var19 = class7.field113[var8.field1426] * var18 >> 16;
		int var20 = class7.field121[var8.field1426] * var18 >> 16;
		var11.method804();
		var11.method813(var8.field1416, var8.field1374, var8.field1426, var8.field1384, var8.field1376 + var19 + var11.field470 / 2, var8.field1376 + var20);
		if (arg0 >= 1) {
			var17.method378(1);
		}
		if (arg0 >= 2) {
			var17.method378(16777215);
		}
		if (arg3 != 0) {
			var17.method373(arg3);
		}
		class141.method1052(var17.field1144, 36, 32);
		if (var8.field1388 != -1) {
			var12.method369(0, 0);
		}
		if (!arg2 && (var8.field1409 == 1 || arg4 != 1) && arg4 != -1) {
			class54.field1278.method791(class74.method598(arg4), 0, 9, 16776960, 1);
		}
		if (!arg2) {
			class47.field1114.method484(var5, var17);
		}
		class141.method1052(var13, var14, var15);
		class141.method1061(var16);
		class7.method36();
		class7.field118 = true;
		return var17;
	}

	@ObfuscatedName("ta.a(B)V")
	public static void method1018() {
		class154.field3529 = false;
		class150.field3427 = false;
	}

	@ObfuscatedName("ta.a(ILjd;)V")
	public void method1013(class66 arg0) {
		while (true) {
			int var2 = arg0.method532();
			if (var2 == 0) {
				return;
			}
			this.method1015(var2, arg0);
		}
	}

	@ObfuscatedName("ta.a(ZILjd;)V")
	public void method1015(int arg0, class66 arg1) {
		if (arg0 == 1) {
			this.field3039 = arg1.method531();
		} else if (arg0 == 2) {
			this.field3096 = arg1.method531();
		} else if (arg0 == 4) {
			this.field3079 = arg1.method531();
		} else if (arg0 == 5) {
			this.field3074 = arg1.method531();
		} else if (arg0 == 6) {
			this.field3059 = arg1.method531();
		} else if (arg0 == 7) {
			this.field3073 = arg1.method532();
		} else if (arg0 == 8) {
			this.field3064 = arg1.method532();
		} else if (arg0 == 40) {
			int var5 = arg1.method532();
			this.field3071 = new short[var5];
			this.field3055 = new short[var5];
			for (int var6 = 0; var6 < var5; var6++) {
				this.field3071[var6] = (short) arg1.method531();
				this.field3055[var6] = (short) arg1.method531();
			}
		} else if (arg0 == 41) {
			int var3 = arg1.method532();
			this.field3104 = new short[var3];
			this.field3102 = new short[var3];
			for (int var4 = 0; var4 < var3; var4++) {
				this.field3102[var4] = (short) arg1.method531();
				this.field3104[var4] = (short) arg1.method531();
			}
			return;
		}
	}

	@ObfuscatedName("ta.c(II)Lod;")
	public class101 method1017(int arg0) {
		class101 var2 = (class101) class47.field1115.method485((long) this.field3070);
		if (var2 == null) {
			class104 var3 = class104.method840(class158.field3634, this.field3039);
			if (var3 == null) {
				return null;
			}
			if (this.field3071 != null) {
				for (int var4 = 0; var4 < this.field3071.length; var4++) {
					var3.method833(this.field3071[var4], this.field3055[var4]);
				}
			}
			if (this.field3102 != null) {
				for (int var5 = 0; var5 < this.field3102.length; var5++) {
					var3.method853(this.field3102[var5], this.field3104[var5]);
				}
			}
			var2 = var3.method847(this.field3073 + 64, this.field3064 + 850, -30, -50, -30);
			class47.field1115.method484((long) this.field3070, var2);
		}
		class101 var6;
		if (this.field3096 == -1 || arg0 == -1) {
			var6 = var2.method820(true);
		} else {
			var6 = class97.method760(this.field3096).method1111(var2, arg0);
		}
		if (this.field3079 != 128 || this.field3074 != 128) {
			var6.method805(this.field3079, this.field3074, this.field3079);
		}
		if (this.field3059 != 0) {
			if (this.field3059 == 90) {
				var6.method801();
			}
			if (this.field3059 == 180) {
				var6.method801();
				var6.method801();
			}
			if (this.field3059 == 270) {
				var6.method801();
				var6.method801();
				var6.method801();
			}
		}
		return var6;
	}
}
