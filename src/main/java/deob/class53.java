package deob;

@ObfuscatedName("hd")
public final class class53 extends class13 {

	@ObfuscatedName("hd.ab")
	public int field1209 = -1;

	@ObfuscatedName("hd.ac")
	public short[] field1261;

	@ObfuscatedName("hd.bb")
	public boolean field1210 = false;

	@ObfuscatedName("hd.bc")
	public int field1262 = 128;

	@ObfuscatedName("hd.cb")
	public short[] field1211;

	@ObfuscatedName("hd.db")
	public static class32 field1212 = class73.method593("Ok", true);

	@ObfuscatedName("hd.eb")
	public class32 field1213 = class115.field2699;

	@ObfuscatedName("hd.fb")
	public static class18 field1214;

	@ObfuscatedName("hd.hb")
	public int field1216 = 128;

	@ObfuscatedName("hd.ib")
	public static int field1217 = 0;

	@ObfuscatedName("hd.jb")
	public int[] field1218;

	@ObfuscatedName("hd.kb")
	public boolean field1219 = true;

	@ObfuscatedName("hd.lb")
	public static int field1220 = 0;

	@ObfuscatedName("hd.mb")
	public int field1221;

	@ObfuscatedName("hd.nb")
	public int field1222 = 1;

	@ObfuscatedName("hd.ob")
	public int field1223 = 0;

	@ObfuscatedName("hd.qb")
	public int field1225 = -1;

	@ObfuscatedName("hd.rb")
	public int field1226 = -1;

	@ObfuscatedName("hd.wb")
	public int field1231 = 0;

	@ObfuscatedName("hd.L")
	public static int[] field1195 = new int[50];

	@ObfuscatedName("hd.Z")
	public static int field1208 = 1;

	@ObfuscatedName("hd.Cb")
	public static class32 field1237 = class73.method593("Gegenstand f-Ur Mitglieder", true);

	@ObfuscatedName("hd.Nb")
	public static long field1248 = 0L;

	@ObfuscatedName("hd.Eb")
	public static int[][] field1239 = new int[104][104];

	@ObfuscatedName("hd.Xb")
	public static class32 field1258 = class73.method593("Login limit exceeded)3", true);

	@ObfuscatedName("hd.Ob")
	public static int field1249 = 0;

	@ObfuscatedName("hd.zb")
	public static class32 field1234 = field1258;

	@ObfuscatedName("hd.T")
	public static class32 field1202 = field1212;

	@ObfuscatedName("hd.V")
	public boolean field1204 = false;

	@ObfuscatedName("hd.J")
	public int field1193 = -1;

	@ObfuscatedName("hd.Ab")
	public int field1235 = 2;

	@ObfuscatedName("hd.I")
	public int field1192 = 0;

	@ObfuscatedName("hd.Db")
	public int field1238 = 128;

	@ObfuscatedName("hd.Bb")
	public int field1236 = -1;

	@ObfuscatedName("hd.yb")
	public int field1233 = 1;

	@ObfuscatedName("hd.Kb")
	public int field1245 = 16;

	@ObfuscatedName("hd.Fb")
	public int field1240 = 0;

	@ObfuscatedName("hd.W")
	public int field1205 = 0;

	@ObfuscatedName("hd.Rb")
	public int field1252 = -1;

	@ObfuscatedName("hd.Lb")
	public int field1246 = -1;

	@ObfuscatedName("hd.xb")
	public int field1232 = 0;

	@ObfuscatedName("hd.Pb")
	public int field1250 = 0;

	@ObfuscatedName("hd.K")
	public boolean field1194 = false;

	@ObfuscatedName("hd.Tb")
	public int field1254 = -1;

	@ObfuscatedName("hd.Ub")
	public final class32[] field1255 = new class32[5];

	@ObfuscatedName("hd.Sb")
	public boolean field1253 = true;

	@ObfuscatedName("hd.O")
	public boolean field1198 = false;

	@ObfuscatedName("hd.Vb")
	public int field1256 = -1;

	@ObfuscatedName("hd.Wb")
	public boolean field1257 = false;

	@ObfuscatedName("hd.Yb")
	public int field1259 = 0;

	@ObfuscatedName("hd.Hb")
	public int field1242 = 0;

	@ObfuscatedName("hd.P")
	public int[] field1199;

	@ObfuscatedName("hd.Ib")
	public int[] field1243;

	@ObfuscatedName("hd.Qb")
	public int[] field1251;

	@ObfuscatedName("hd.N")
	public short[] field1197;

	@ObfuscatedName("hd.U")
	public short[] field1203;

	@ObfuscatedName("hd.c(B)V")
	public static void method418() {
		class26.field476 = false;
		class119.field2849 = 0;
		class4.field72 = 0;
		class86.field2004 = -1;
		class115.field2700 = 0;
		class44.field1026.field1568 = 0;
		class34.field710 = -1;
		class34.field703 = 0;
		class12.field201 = -1;
		class56.field1294 = 0;
		class158.field3616 = -1;
		class158.field3612.field1568 = 0;
		class45.field1066 = 0;
		for (int var0 = 0; var0 < class52.field1175.length; var0++) {
			if (class52.field1175[var0] != null) {
				class52.field1175[var0].field2756 = -1;
			}
		}
		for (int var1 = 0; var1 < class14.field230.length; var1++) {
			if (class14.field230[var1] != null) {
				class14.field230[var1].field2756 = -1;
			}
		}
		class71.method586();
		class54.method425(30);
		for (int var2 = 0; var2 < 100; var2++) {
			class145.field3297[var2] = true;
		}
	}

	@ObfuscatedName("hd.a(ZI)Lec;")
	public static class32 method420(int arg0) {
		return arg0 < 999999999 ? class159.method1228(arg0) : class158.field3640;
	}

	@ObfuscatedName("hd.a(B)Z")
	public boolean method411() {
		if (this.field1251 == null) {
			return this.field1256 != -1 || this.field1218 != null;
		}
		for (int var1 = 0; var1 < this.field1251.length; var1++) {
			if (this.field1251[var1] != -1) {
				class53 var2 = class5.method22(this.field1251[var1]);
				if (var2.field1256 != -1 || var2.field1218 != null) {
					return true;
				}
			}
		}
		return false;
	}

	@ObfuscatedName("hd.a(IIII[[III)Lod;")
	public class101 method412(int arg0, int arg1, int arg2, int[][] arg3, int arg4, int arg5) {
		long var7;
		if (this.field1199 == null) {
			var7 = arg4 + (this.field1221 << 10);
		} else {
			var7 = arg4 + (arg1 << 3) + (this.field1221 << 10);
		}
		class101 var9 = (class101) class137.field3185.method485(var7);
		if (var9 == null) {
			class104 var10 = this.method413(arg4, arg1);
			if (var10 == null) {
				return null;
			}
			var9 = var10.method847(this.field1231 + 64, this.field1242 * 5 + 768, -50, -10, -50);
			class137.field3185.method484(var7, var9);
		}
		if (this.field1246 >= 0) {
			var9 = var9.method808(arg3, arg0, arg5, arg2, true, this.field1246);
		}
		return var9;
	}

	@ObfuscatedName("hd.a(IZI)Lp;")
	public class104 method413(int arg0, int arg1) {
		class104 var3 = null;
		if (this.field1199 == null) {
			if (arg1 != 10) {
				return null;
			}
			if (this.field1243 == null) {
				return null;
			}
			boolean var8 = this.field1257;
			if (arg1 == 2 && arg0 > 3) {
				var8 = !var8;
			}
			int var9 = this.field1243.length;
			for (int var10 = 0; var10 < var9; var10++) {
				int var11 = this.field1243[var10];
				if (var8) {
					var11 += 65536;
				}
				var3 = (class104) class115.field2720.method485((long) var11);
				if (var3 == null) {
					var3 = class104.method840(class119.field2840, var11 & 0xFFFF);
					if (var3 == null) {
						return null;
					}
					if (var8) {
						var3.method844();
					}
					class115.field2720.method484((long) var11, var3);
				}
				if (var9 > 1) {
					class137.field3186[var10] = var3;
				}
			}
			if (var9 > 1) {
				var3 = new class104(class137.field3186, var9);
			}
		} else {
			int var4 = -1;
			for (int var5 = 0; var5 < this.field1199.length; var5++) {
				if (this.field1199[var5] == arg1) {
					var4 = var5;
					break;
				}
			}
			if (var4 == -1) {
				return null;
			}
			int var6 = this.field1243[var4];
			boolean var7 = arg0 > 3 ^ this.field1257;
			if (var7) {
				var6 += 65536;
			}
			var3 = (class104) class115.field2720.method485((long) var6);
			if (var3 == null) {
				var3 = class104.method840(class119.field2840, var6 & 0xFFFF);
				if (var3 == null) {
					return null;
				}
				if (var7) {
					var3.method844();
				}
				class115.field2720.method484((long) var6, var3);
			}
		}
		boolean var12;
		if (this.field1216 == 128 && this.field1238 == 128 && this.field1262 == 128) {
			var12 = false;
		} else {
			var12 = true;
		}
		boolean var13;
		if (this.field1205 == 0 && this.field1223 == 0 && this.field1232 == 0) {
			var13 = false;
		} else {
			var13 = true;
		}
		class104 var14 = new class104(var3, arg0 == 0 && !var12 && !var13, this.field1203 == null, this.field1211 == null, true);
		if (arg1 == 4 && arg0 > 3) {
			var14.method851();
			var14.method834(45, 0, -45);
		}
		int var15 = arg0 & 0x3;
		if (var15 == 1) {
			var14.method837();
		} else if (var15 == 2) {
			var14.method842();
		} else if (var15 == 3) {
			var14.method838();
		}
		if (this.field1203 != null) {
			for (int var16 = 0; var16 < this.field1203.length; var16++) {
				var14.method833(this.field1203[var16], this.field1261[var16]);
			}
		}
		if (this.field1211 != null) {
			for (int var17 = 0; var17 < this.field1211.length; var17++) {
				var14.method853(this.field1211[var17], this.field1197[var17]);
			}
		}
		if (var12) {
			var14.method855(this.field1216, this.field1238, this.field1262);
		}
		if (var13) {
			var14.method834(this.field1205, this.field1223, this.field1232);
		}
		return var14;
	}

	@ObfuscatedName("hd.e(I)Z")
	public boolean method414() {
		if (this.field1243 == null) {
			return true;
		}
		boolean var1 = true;
		for (int var2 = 0; var2 < this.field1243.length; var2++) {
			var1 &= class119.field2840.method213(this.field1243[var2] & 0xFFFF, 0);
		}
		return var1;
	}

	@ObfuscatedName("hd.a(ZLvc;II[[IIIII)Lod;")
	public class101 method415(class149 arg0, int arg1, int arg2, int[][] arg3, int arg4, int arg5, int arg6, int arg7) {
		long var9;
		if (this.field1199 == null) {
			var9 = (this.field1221 << 10) + arg7;
		} else {
			var9 = (arg2 << 3) + (this.field1221 << 10) + arg7;
		}
		class101 var11 = (class101) class137.field3185.method485(var9);
		if (var11 == null) {
			class104 var12 = this.method413(arg7, arg2);
			if (var12 == null) {
				return null;
			}
			var11 = var12.method847(this.field1231 + 64, this.field1242 * 5 + 768, -50, -10, -50);
			class137.field3185.method484(var9, var11);
		}
		if (arg0 == null && this.field1246 == -1) {
			return var11;
		}
		class101 var13;
		if (arg0 == null) {
			var13 = var11.method799(true);
		} else {
			var13 = arg0.method1109(arg7, arg5, var11);
		}
		if (this.field1246 >= 0) {
			var13 = var13.method808(arg3, arg4, arg6, arg1, false, this.field1246);
		}
		return var13;
	}

	@ObfuscatedName("hd.a(BLjd;I)V")
	public void method416(class66 arg0, int arg1) {
		if (arg1 == 1) {
			int var3 = arg0.method532();
			if (var3 > 0) {
				if (this.field1243 != null && !class37.field888) {
					arg0.field1568 += var3 * 3;
					return;
				}
				this.field1243 = new int[var3];
				this.field1199 = new int[var3];
				for (int var4 = 0; var4 < var3; var4++) {
					this.field1243[var4] = arg0.method531();
					this.field1199[var4] = arg0.method532();
				}
				return;
			}
		} else if (arg1 == 2) {
			this.field1213 = arg0.method555();
		} else if (arg1 == 5) {
			int var13 = arg0.method532();
			if (var13 > 0) {
				if (this.field1243 != null && !class37.field888) {
					arg0.field1568 += var13 * 2;
					return;
				}
				this.field1199 = null;
				this.field1243 = new int[var13];
				for (int var14 = 0; var14 < var13; var14++) {
					this.field1243[var14] = arg0.method531();
				}
				return;
			}
		} else if (arg1 == 14) {
			this.field1222 = arg0.method532();
			return;
		} else if (arg1 == 15) {
			this.field1233 = arg0.method532();
			return;
		} else if (arg1 == 17) {
			this.field1219 = false;
			this.field1235 = 0;
			return;
		} else if (arg1 == 18) {
			this.field1219 = false;
			return;
		} else if (arg1 == 19) {
			this.field1193 = arg0.method532();
			return;
		} else if (arg1 == 21) {
			this.field1246 = 0;
			return;
		} else if (arg1 == 22) {
			this.field1194 = true;
			return;
		} else if (arg1 == 23) {
			this.field1210 = true;
			return;
		} else {
			if (arg1 == 24) {
				this.field1225 = arg0.method531();
				if (this.field1225 == 65535) {
					this.field1225 = -1;
					return;
				}
			} else if (arg1 == 27) {
				this.field1235 = 1;
			} else if (arg1 == 28) {
				this.field1245 = arg0.method532();
				return;
			} else if (arg1 == 29) {
				this.field1231 = arg0.method544();
				return;
			} else if (arg1 == 39) {
				this.field1242 = arg0.method544() * 5;
				return;
			} else {
				if (arg1 >= 30 && arg1 < 35) {
					this.field1255[arg1 - 30] = arg0.method555();
					if (this.field1255[arg1 - 30].method271(class159.field3655)) {
						this.field1255[arg1 - 30] = null;
						return;
					}
				} else if (arg1 == 40) {
					int var11 = arg0.method532();
					this.field1203 = new short[var11];
					this.field1261 = new short[var11];
					for (int var12 = 0; var12 < var11; var12++) {
						this.field1203[var12] = (short) arg0.method531();
						this.field1261[var12] = (short) arg0.method531();
					}
				} else if (arg1 == 41) {
					int var5 = arg0.method532();
					this.field1197 = new short[var5];
					this.field1211 = new short[var5];
					for (int var6 = 0; var6 < var5; var6++) {
						this.field1211[var6] = (short) arg0.method531();
						this.field1197[var6] = (short) arg0.method531();
					}
					return;
				} else if (arg1 == 60) {
					this.field1209 = arg0.method531();
					return;
				} else if (arg1 == 62) {
					this.field1257 = true;
					return;
				} else if (arg1 == 64) {
					this.field1253 = false;
					return;
				} else if (arg1 == 65) {
					this.field1216 = arg0.method531();
					return;
				} else if (arg1 == 66) {
					this.field1238 = arg0.method531();
					return;
				} else if (arg1 == 67) {
					this.field1262 = arg0.method531();
					return;
				} else if (arg1 == 68) {
					this.field1226 = arg0.method531();
					return;
				} else if (arg1 == 69) {
					this.field1192 = arg0.method532();
					return;
				} else if (arg1 == 70) {
					this.field1205 = arg0.method551();
					return;
				} else if (arg1 == 71) {
					this.field1223 = arg0.method551();
					return;
				} else if (arg1 == 72) {
					this.field1232 = arg0.method551();
					return;
				} else if (arg1 == 73) {
					this.field1204 = true;
					return;
				} else if (arg1 == 74) {
					this.field1198 = true;
					return;
				} else if (arg1 == 75) {
					this.field1252 = arg0.method532();
					return;
				} else if (arg1 == 77) {
					this.field1254 = arg0.method531();
					if (this.field1254 == 65535) {
						this.field1254 = -1;
					}
					this.field1236 = arg0.method531();
					if (this.field1236 == 65535) {
						this.field1236 = -1;
					}
					int var7 = arg0.method532();
					this.field1251 = new int[var7 + 1];
					for (int var8 = 0; var8 <= var7; var8++) {
						this.field1251[var8] = arg0.method531();
						if (this.field1251[var8] == 65535) {
							this.field1251[var8] = -1;
						}
					}
					return;
				} else if (arg1 == 78) {
					this.field1256 = arg0.method531();
					this.field1240 = arg0.method532();
					return;
				} else {
					if (arg1 == 79) {
						this.field1259 = arg0.method531();
						this.field1250 = arg0.method531();
						this.field1240 = arg0.method532();
						int var9 = arg0.method532();
						this.field1218 = new int[var9];
						for (int var10 = 0; var10 < var9; var10++) {
							this.field1218[var10] = arg0.method531();
						}
					} else if (arg1 == 81) {
						this.field1246 = arg0.method532() * 256;
						return;
					}
					return;
				}
				return;
			}
			return;
		}
	}

	@ObfuscatedName("hd.a(Ljd;Z)V")
	public void method419(class66 arg0) {
		while (true) {
			int var2 = arg0.method532();
			if (var2 == 0) {
				return;
			}
			this.method416(arg0, var2);
		}
	}

	@ObfuscatedName("hd.a(ZIII[[III)Ldd;")
	public class26 method421(int arg0, int arg1, int arg2, int[][] arg3, int arg4, int arg5) {
		long var7;
		if (this.field1199 == null) {
			var7 = arg2 + (this.field1221 << 10);
		} else {
			var7 = (this.field1221 << 10) + (arg5 << 3) + arg2;
		}
		class26 var9 = (class26) class33.field689.method485(var7);
		if (var9 == null) {
			class104 var10 = this.method413(arg2, arg5);
			if (var10 == null) {
				return null;
			}
			if (this.field1194) {
				var9 = var10;
				var10.field2457 = (short) (this.field1242 * 5 + 768);
				var10.field2446 = (short) (this.field1231 + 64);
				var10.method839();
			} else {
				var9 = var10.method847(this.field1231 + 64, this.field1242 * 5 + 768, -50, -10, -50);
			}
			class33.field689.method484(var7, var9);
		}
		if (this.field1194) {
			var9 = ((class104) var9).method832();
		}
		if (this.field1246 >= 0) {
			if (var9 instanceof class101) {
				var9 = ((class101) var9).method808(arg3, arg4, arg1, arg0, true, this.field1246);
			} else if (var9 instanceof class104) {
				var9 = ((class104) var9).method836(arg3, arg4, arg1, arg0, this.field1246);
			}
		}
		return var9;
	}

	@ObfuscatedName("hd.d(B)Lhd;")
	public class53 method422() {
		int var1 = -1;
		if (this.field1254 != -1) {
			var1 = class70.method577(this.field1254);
		} else if (this.field1236 != -1) {
			var1 = class27.field504[this.field1236];
		}
		return var1 < 0 || var1 >= this.field1251.length || this.field1251[var1] == -1 ? null : class5.method22(this.field1251[var1]);
	}

	@ObfuscatedName("hd.b(II)Z")
	public boolean method423(int arg0) {
		if (this.field1199 != null) {
			for (int var2 = 0; var2 < this.field1199.length; var2++) {
				if (this.field1199[var2] == arg0) {
					return class119.field2840.method213(this.field1243[var2] & 0xFFFF, 0);
				}
			}
			return true;
		} else if (this.field1243 == null) {
			return true;
		} else if (arg0 == 10) {
			boolean var3 = true;
			for (int var4 = 0; var4 < this.field1243.length; var4++) {
				var3 &= class119.field2840.method213(this.field1243[var4] & 0xFFFF, 0);
			}
			return var3;
		} else {
			return true;
		}
	}

	@ObfuscatedName("hd.f(I)V")
	public void method424() {
		if (this.field1193 == -1) {
			this.field1193 = 0;
			if (this.field1243 != null && (this.field1199 == null || this.field1199[0] == 10)) {
				this.field1193 = 1;
			}
			for (int var1 = 0; var1 < 5; var1++) {
				if (this.field1255[var1] != null) {
					this.field1193 = 1;
				}
			}
		}
		if (this.field1252 == -1) {
			this.field1252 = this.field1235 == 0 ? 0 : 1;
		}
	}
}
