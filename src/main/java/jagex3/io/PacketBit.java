package jagex3.io;

import deob.ObfuscatedName;

@ObfuscatedName("fe")
public final class PacketBit extends Packet {

    @ObfuscatedName("fd.Eb")
    public static int[] BITMASK = new int[]{0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535, 131071, 262143, 524287, 1048575, 2097151, 4194303, 8388607, 16777215, 33554431, 67108863, 134217727, 268435455, 536870911, 1073741823, Integer.MAX_VALUE, -1};
    @ObfuscatedName("fe.Dc")
	public int bitPos;

	@ObfuscatedName("fe.yc")
	public Isaac random;

	@ObfuscatedName("fe.m(II)I")
	public int bitsLeft(int arg0) {
		return arg0 * 8 - this.bitPos;
	}

	@ObfuscatedName("fe.l(B)V")
	public void gBitEnd() {
		super.pos = (this.bitPos + 7) / 8;
	}

	@ObfuscatedName("fe.m(B)I")
	public int g1Enc() {
		return super.data[super.pos++] - this.random.takeNextValue() & 0xFF;
	}

	@ObfuscatedName("fe.e(IB)I")
	public int gBit(int arg0) {
		int var2 = this.bitPos >> 3;
		int var3 = 0;
		int var4 = 8 - (this.bitPos & 0x7);
		this.bitPos += arg0;
		while (var4 < arg0) {
			var3 += (BITMASK[var4] & super.data[var2++]) << arg0 - var4;
			arg0 -= var4;
			var4 = 8;
		}
		int var5;
		if (arg0 == var4) {
			var5 = (super.data[var2] & BITMASK[var4]) + var3;
		} else {
			var5 = (BITMASK[arg0] & super.data[var2] >> var4 - arg0) + var3;
		}
		return var5;
	}

	public PacketBit(int arg0) {
		super(arg0);
	}

	@ObfuscatedName("fe.n(II)V")
	public void p1Enc(int arg0) {
		super.data[super.pos++] = (byte) (this.random.takeNextValue() + arg0);
	}

	@ObfuscatedName("fe.a(I[I)V")
	public void seed(int[] arg0) {
		this.random = new Isaac(arg0);
	}

	@ObfuscatedName("fe.p(I)V")
	public void gBitStart() {
		this.bitPos = super.pos * 8;
	}
}
